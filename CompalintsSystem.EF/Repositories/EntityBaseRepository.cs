...
namespace CompalintsSystem.EF.Repositories
{
    ...
    params Expression<Func<T, object>>[] includeproperties)
        ...
        public async Task<SelectDataCommuncationDropdownsVM> GetAndAddCommunicationDropdownsValuesForBeneficaie(int subDirctoty, int DepartmentsId, int CollegesId, string role, string roleId)
        {
            var responseSub = new SelectDataCommuncationDropdownsVM
            {
                ...
                ApplicationUsers = await _context.Users
                   .Join(_context.UserRoles, u => u.Id, ur => ur.UserId, (u, ur) => new { User = u, UserRole = ur })
                   .Where(x => x.UserRole.RoleId == "901d057e-c4d6-41e1-8616-fc74da8e16da"//  عرض جميع المستخدمين في المستوى التابعة 
                       && x.User.SubDepartmentsId == subDirctoty)
                   .OrderBy(x => x.User.FullName)
                   .Select(x => x.User)
                   .ToListAsync(),
